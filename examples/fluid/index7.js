...
function main({ pane, contextID, glslVersion }) {
    ...
    const PARAMS = {
        ...
        render: 'Fluid',
        particleDensity: 0.1,
        maxVelocity: 5,          // Lowered from 30 to 5
        touchForceScale: 0.5,    // Lowered from 2 to 0.5
    };
    ...
    function applyAudioForces(frequencyRanges) {
        ...
        // Apply multiple small forces at random positions
        const numForces = 5; // Number of random forces to apply
        for (let i = 0; i < numForces; i++) {
            ...
            composer.stepCircle({
                ...
                position,
                diameter: 100, // Adjusted to use 'diameter' instead of 'radius'
            });
        }
    }
    ...
    const advectParticles = new GPUProgram(composer, {
        ...
        void main() {
            ...
            // Apply non-linear damping to velocity
            float speed = length(velocity);
            float dampingFactor = 1.0 / (1.0 + speed * 0.1); // Adjust 0.1 to control damping strength
            velocity *= dampingFactor;
    
            vec2 halfStep = position + velocity * 0.5 * ${1 / NUM_RENDER_STEPS};
            vec2 velocity2 = texture(u_velocity, halfStep * pxSize).xy * dampingFactor;
