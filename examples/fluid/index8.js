...
function main({ pane, contextID, glslVersion }) {
    ...
    const PARTICLE_DENSITY = PARAMS.particleDensity;
    ...
    function applyAudioForces(frequencyRanges) {
        ...
        const lowForce = mapRange(low, 0, 1, 0, currentMaxVelocity); // Bass
        const midForce = mapRange(mid, 0, 1, 0, currentMaxVelocity / 2); // Mid frequencies
        const highForce = mapRange(high, 0, 1, 0, currentMaxVelocity / 4); // High frequencies
