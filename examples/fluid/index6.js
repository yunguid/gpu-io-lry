...
function main({ pane, contextID, glslVersion }) {
    ...
    const MAX_NUM_PARTICLES = 200000; // Increased maximum for dynamic adjustment
    ...
    const MAX_FORCE = 100; // Increased upper limit for force magnitudes
    ...
    function setupAudioProcessing() {
        ...
        analyserNode.fftSize = 4096; // Higher FFT size for better frequency resolution
        analyserNode.smoothingTimeConstant = 0.8; // Smooths the FFT data
    ...
    function visualizeAudio() {
        ...
        // Dynamic particle count based on overall energy
        adjustParticleCount();
    }
    ...
    function detectBeats(currentSpectrum) {
        const currentTime = performance.now();
        const threshold = PARAMS.beatDetection.sensitivity;
        let flux = 0;

        for (let i = 0; i < currentSpectrum.length; i++) {
            const magnitude = currentSpectrum[i] / 255;
            flux += magnitude * magnitude;
        }

        // Simple flux-based beat detection
        if (flux > threshold && (currentTime - lastBeatTime) > PARAMS.beatDetection.cooldown) {
            beatDetected = true;
    ...
    // Function to handle beat events
    function onBeat() {
        // Increase force scales during beats
        TOUCH_FORCE_SCALE *= 1.2;
        MAX_VELOCITY *= 1.1;

        // Reset to normal after a short duration
        setTimeout(() => {
            TOUCH_FORCE_SCALE /= 1.2;
            MAX_VELOCITY /= 1.1;
        }, 200);
    ...
    const renderParticles = new GPUProgram(composer, {
        ...
        void main() {
            ...
            float opacity = mix(0.0, 1.0, smoothstep(0.0, 0.1, ageFraction)) * mix(1.0, 0.0, smoothstep(0.9, 1.0, ageFraction));
    ...
    const COLOR_SMOOTHING = PARAMS.colorModulation.smoothing; // Adjust between 0 (no smoothing) to 1 (instant change)

    // Initialize the sensitivity and modulation folders in the UI
    ...
    // Initialize additional UI folders
    const beatFolder = pane.addFolder({ title: 'Beat Detection' });
    beatFolder.addInput(PARAMS.beatDetection, 'sensitivity', { min: 0.5, max: 5.0, step: 0.1, label: 'Beat Sensitivity' });
    beatFolder.addInput(PARAMS.beatDetection, 'cooldown', { min: 100, max: 1000, step: 50, label: 'Beat Cooldown (ms)' });

    const modulationFolder = pane.addFolder({ title: 'Color Modulation' });
    modulationFolder.addInput(PARAMS.colorModulation, 'enable', { label: 'Enable Color Modulation' });
    modulationFolder.addInput(PARAMS.colorModulation, 'smoothing', { min: 0.0, max: 1.0, step: 0.05, label: 'Color Smoothing' });

    const interactionFolder = pane.addFolder({ title: 'Particle Interaction' });
    interactionFolder.addInput(PARAMS.particleInteraction, 'enable', { label: 'Enable Particle Interaction' });
    interactionFolder.addInput(PARAMS.particleInteraction, 'strength', { min: 0.0, max: 5.0, step: 0.1, label: 'Interaction Strength' });
    ...
    function loop() {
        ...
        if (isPlaying) {
            visualizeAudio();
            if (PARAMS.colorModulation.enable) {
                updateColor(frequencyRanges);
            }
            if (PARAMS.particleInteraction.enable) {
                adjustParticleInteractions();
            }
        }
        ...
        if (shouldSavePNG) {
            composer.savePNG({ filename: `fluid_${Date.now()}` });
            shouldSavePNG = false;
        }
    }
    ...
    function adjustParticleLifetime(frequencyRanges) {
        ...
        PARTICLE_LIFETIME = PARTICLE_LIFETIME_BASE * (1 + low * 2); // Longer lifetimes for stronger low frequencies
    ...
    // Function to adjust particle interactions based on audio features
    function adjustParticleInteractions() {
        // Example: Increase interaction strength during beats
        if (beatDetected) {
            // Implement interaction logic, e.g., applying attractive/repulsive forces
            // This requires additional GPU programs and shaders
            // Placeholder for interaction adjustment
            console.log('Beat detected! Adjusting particle interactions.');
            // Adjust interaction parameters or apply additional forces
        }
    }

    // Function to dynamically adjust particle count based on audio energy
    function adjustParticleCount() {
        const energy = frequencyRanges.low + frequencyRanges.mid + frequencyRanges.high;
        const desiredDensity = mapRange(energy, 0, 3, 0.05, 0.3); // Adjust min and max density as needed

        if (desiredDensity !== PARTICLE_DENSITY) {
            PARTICLE_DENSITY = desiredDensity;
            onResize();
        }
    }
